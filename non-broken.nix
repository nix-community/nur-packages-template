  ...
  (
    ...
    && !(
      (builtins.hasAttr "meta" p) && (
        ((builtins.hasAttr "broken" p.meta) && (p.meta.broken)) || ((builtins.hasAttr "platforms" p.meta) && !(pkgs.lib.any (pkgs.lib.meta.platformMatch pkgs.stdenv.hostPlatform) p.meta.platforms))
      )
    )
  )
